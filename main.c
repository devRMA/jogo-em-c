...
// para limpar a tela a sé usar system("cls");

#define nome_do_jogo "Projeto" //constante que guarda o nome do jogo

//Constantes que guardam o código de cada cor(usado no procedimento mudar_cor)
...
#define roxo 13
#define amarelo 14
//variavel que vai ficar responsável por ficar recebendo a escolha do player
int escolha = 0;
...
void mudar_cor(int cor) //função para mudar a cor do print
{
    ...
    switch (cor) //verifica qual cor foi passado como parametro
    {
        case 4: //se o que foi digitado for 4(vermelho escuro):
            num_cor = 4; //a variavel vai receber o código conforme o comentario anterior
            break; //pula a comparação com os outros casos
        case 8: //se o que foi digitado for 8(cinza):
            num_cor = 8; //a variavel vai receber o código conforme o comentario anterior
            break; //pula a comparação com os outros casos
        case 9:
            num_cor = 9;
            break;
    ...
    //código para mudar a cor do print
...
{
    int quantas_casas_tem_entre_os_players = (pos_p2 - pos_p1); //pega a distancia entre os players(o número de casas entre eles)
    ...
    if (quantas_casas_tem_entre_os_players <= 3 && quantas_casas_tem_entre_os_players > 0)
    {
        ...
        else //senão, a variavel do player 2, fica True
        {
            p2_atacando = 1;
            return 1; //se o player conseguiu atacar
        }
    ...
    if(ataque_p1){ // se o player 1 atacar, ele pede uma reação do player 2.
        ...
        if(! p2_defendendo) //se o player 2 não estiver e defendendo
        {
            ...
            if (escolha == 1) //escolheu avançar
            {
                ...
                scanf("%d", &escolha); //recebe quantas casas o player quer avançar
...
void status() //um procedimento que da print nos status
{
    ...
    mudar_cor(celeste);
    printf("\n\\");
    for (int c = 1; c <= 50; c++)//processo para alterar o ícone do personagem se estiver se defendendo
    {
        if(c == pos_p1)
        {
            if (p1_defendendo)
...
void player1()
{
    ...
    {
        if (atacar(1)) //se a função retornou true, é pq o ataque é possível, logo a variavel global foi mudada
        {
...
void player2()
{
    ...
    {
        if (atacar(2)) //se a função retornou true, é pq o ataque é possível, logo a variavel global foi mudada
        {
            ataque(p1_atacando, p2_atacando);
            Sleep(2000);
        }
...
/*
código que da print em todas as cores e seus respectivos códigos:
...
int main()
{
   ...
   {
       mudar_cor(c);
       printf("Print colorido com o código %d\n", c);}
    return 0;
}
*/
